        ...
        with tqdm(total=n_train, desc=f'Epoch {epoch}/{epochs}', unit='img') as pbar:
            for batch in train_loader:
                ...
                if true_masks.max() >= model.n_classes:
                    logging.error(f'Label values exceed the number of classes: {true_masks.max()}')
                    raise ValueError(f'Label values exceed the number of classes: {true_masks.max()}')

                assert images.shape[1] == model.n_channels, \
                    f'Network defined with {model.n_channels} input channels, but loaded images have {images.shape[1]} channels.'
...
if __name__ == '__main__':
    ...
    try:
        ...
        logging.error('Detected OutOfMemoryError! Consider reducing batch size or image scale.')
